...
export default function AdminPage() {
  ...
  const dashRows = todaySchedules.map(sc => {
    ...
    if (record?.check_out) {
      ...
      color = stClr[record.status] || '#10b981'
      ...
      if (start) {
        ...
        if (diffMins < 0) {
          color = '#8892a8'; bg = 'rgba(136,146,168,.1)'; statusLabel = 'Esperado'
        } else if (diffMins <= grace) {
          color = '#f59e0b'; bg = 'rgba(245,158,11,.12)'; statusLabel = 'En tolerancia'
        } else if (diffMins <= absent) {
          color = '#f59e0b'; bg = 'rgba(245,158,11,.12)'; statusLabel = 'Tolerancia vencida'
        } else {
          color = '#ef4444'; bg = 'rgba(239,68,68,.12)'; statusLabel = 'No se presentó'
        }
      } else {
        color = '#8892a8'; bg = 'rgba(136,146,168,.1)'; statusLabel = 'Esperado'
      }
  ...
  async function delSite(id) {
    ...
    setToast('Sitio eliminado'); setModal(null); load()
  }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', fontFamily: "'DM Sans', sans-serif", background: '#0a0e1a', color: '#f1f5f9' }}>
        ...
        <div style={{ flex: 1, padding: '18px 22px', overflow: 'auto' }}>
          ...
          {tab === 'dashboard' && <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, marginBottom: 18 }}>
              {[
                ...
                ['Con retardo', statTol,    '#f59e0b'],
                ['Completaron', statDone,   '#3b82f6'],
                ['No llegaron', statMissing,'#ef4444'],
                ['Por llegar',  statPending,'#8892a8'],
              ].map(([l, v, c]) => (
          ...
          {tab === 'attendance' && <>
            ...
            <div style={{ background: '#1a2035', border: '1px solid #1e2a45', borderRadius: 10, padding: '14px 16px', marginBottom: 14, display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10 }}>
              ...
              <div>
                <div style={{ fontSize: 10, color: '#4a5568', marginBottom: 4, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.5px' }}>Sucursal</div>
              ...
              <div>
                <div style={{ fontSize: 10, color: '#4a5568', marginBottom: 4, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.5px' }}>Desde</div>
                <input type='date' value={filterFrom} onChange={e => setFilterFrom(e.target.value)} style={inputStyle} />
              </div>
              <div>
                <div style={{ fontSize: 10, color: '#4a5568', marginBottom: 4, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.5px' }}>Hasta</div>
            ...
            <div style={{ background: '#1a2035', border: '1px solid #1e2a45', borderRadius: 10, overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>{['Fecha', 'Empleado', 'Sucursal', 'Entrada', 'Salida', 'Horas', 'Ventas', 'Estado'].map(h => (
                ...
                <tbody>
                  ...
                  {filteredAtt.length === 0 && (
                    <tr><td colSpan={8} style={{ padding: 24, textAlign: 'center', color: '#4a5568', fontSize: 12 }}>Sin registros con los filtros seleccionados</td></tr>
                  )}
                </tbody>
              </table>
            </div>
      ...
      {sideEmp && (
        <EmpSidePanel
          emp={sideEmp}
          att={att.filter(r => r.employee_id === sideEmp.id)}
          sites={sites}
  ...
  return (
    <div style={{ position: 'fixed', top: 0, right: 0, width: 480, height: '100vh', background: '#111827', borderLeft: '1px solid #1e2a45', display: 'flex', flexDirection: 'column', zIndex: 150, boxShadow: '-8px 0 32px rgba(0,0,0,.4)' }}>
      ...
      <div style={{ padding: '12px 20px', borderBottom: '1px solid #1e2a45', display: 'flex', gap: 10, alignItems: 'center', flexShrink: 0 }}>
        ...
        {(from || to) && (
          <button onClick={() => { setFrom(''); setTo('') }} style={{ background: 'none', border: '1px solid #1e2a45', borderRadius: 5, color: '#8892a8', fontSize: 10, padding: '5px 10px', cursor: 'pointer', fontFamily: 'inherit', marginTop: 14 }}>Limpiar</button>
        )}
      </div>
      ...
      {totalSales > 0 && (
        <div style={{ padding: '10px 20px', borderBottom: '1px solid #1e2a45', display: 'flex', gap: 20, flexShrink: 0 }}>
          <div>
            <span style={{ fontSize: 10, color: '#4a5568', textTransform: 'uppercase', letterSpacing: '.5px', marginRight: 8 }}>Ventas</span>
            <span style={{ fontSize: 14, fontWeight: 700, fontFamily: "'JetBrains Mono'", color: '#10b981' }}>${Number(totalSales).toLocaleString('es-MX')}</span>
          </div>
          <div>
            <span style={{ fontSize: 10, color: '#4a5568', textTransform: 'uppercase', letterSpacing: '.5px', marginRight: 8 }}>Prom. diario</span>
            <span style={{ fontSize: 13, fontWeight: 600, fontFamily: "'JetBrains Mono'", color: '#10b981' }}>${Number(totalSales / Math.max(filtered.filter(r => r.sales_amount > 0).length, 1)).toLocaleString('es-MX', { maximumFractionDigits: 0 })}</span>
          </div>
        </div>
      )}
...
function ScheduleModal({ emp, sites, schedules, onSave, onClose }) {
  ...
  const copyToAll = (srcDate) => {
    const src = week[srcDate]
    if (!src?.on) return
    setWeek(p => {
      const nw = { ...p }
      weekDates.forEach(({ date }) => {
        if (nw[date]?.on && date !== srcDate) {
          nw[date] = { ...nw[date], site_id: src.site_id, start_time: src.start_time, end_time: src.end_time, lunch_mins: src.lunch_mins }
        }
      })
      return nw
    })
  ...
  const weekLabel = (() => {
    const s = weekDates[0].d
    const e = weekDates[6].d
